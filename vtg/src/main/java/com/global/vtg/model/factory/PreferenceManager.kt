...
class PreferenceManager constructor(private var sharedPreferences: SharedPreferences) :
    ...
    companion object {
        ...
        const val KEY_IS_CHILD = "pre.key.child"
        const val KEY_USER_MAIN = "pre.key.user.main"
        const val KEY_USER_NAME_PARENT = "pre.key.userName_parent"
        const val KEY_PASSWORD = "pre.key.password"
        const val KEY_PASSWORD_PARENT = "pre.key.password_parent"
        const val KEY_ROLE = "pre.key.role"
        const val KEY_LAN_CODE = "pre.key.lan_code"
        const val KEY_TOKEN = "pre.key.token"
